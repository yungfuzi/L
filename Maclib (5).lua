...
local Themes = {
	...
	-- ============================== NEW THEMES ==============================
	Amethyst = {
		BaseBackground = Color3.fromRGB(17, 14, 23),
		SidebarBackground = Color3.fromRGB(23, 18, 31),
		DividerColor = Color3.fromRGB(190, 160, 255),
		DividerTransparency = 0.88,

		TextPrimary = Color3.fromRGB(240, 235, 255),
		TextPrimaryTransparency = 0.1,
		TextMuted = Color3.fromRGB(200, 180, 255),
		TextMutedTransparency = 0.55,

		StrokeColor = Color3.fromRGB(170, 130, 255),
		StrokeTransparency = 0.85,

		ElementBackground = Color3.fromRGB(170, 130, 255),
		ElementBackgroundTransparency = 0.95,

		ToggleOff = Color3.fromRGB(70, 60, 90),
		GlobalSettingsBackground = Color3.fromRGB(17, 14, 23)
	},
	Ocean = {
		BaseBackground = Color3.fromRGB(10, 16, 22),
		SidebarBackground = Color3.fromRGB(14, 22, 30),
		DividerColor = Color3.fromRGB(120, 200, 255),
		DividerTransparency = 0.88,

		TextPrimary = Color3.fromRGB(230, 245, 255),
		TextPrimaryTransparency = 0.1,
		TextMuted = Color3.fromRGB(150, 200, 230),
		TextMutedTransparency = 0.5,

		StrokeColor = Color3.fromRGB(90, 180, 255),
		StrokeTransparency = 0.85,

		ElementBackground = Color3.fromRGB(90, 180, 255),
		ElementBackgroundTransparency = 0.95,

		ToggleOff = Color3.fromRGB(50, 70, 85),
		GlobalSettingsBackground = Color3.fromRGB(10, 16, 22)
	}
	-- ========================== END NEW THEMES ==============================
}
...
-- Creates an optional icon for an element/tab.
-- If Settings.Border is true, the icon is placed inside a rounded square
-- background (iOS app-icon style) colored with Settings.Color (hex supported).
-- Returns the created instance (or nil if no Icon was provided) and the
-- total horizontal space (in pixels) it occupies, including spacing.
local function CreateElementIcon(parent, Settings, size, leftOffset, vertAnchor)
	...
	vertAnchor = vertAnchor or 0.5

	if Settings.Border then
		local iconBackground = Instance.new("Frame")
		iconBackground.Name = "IconBackground"
		iconBackground.AnchorPoint = Vector2.new(0, vertAnchor)
		iconBackground.Position = UDim2.new(0, leftOffset, vertAnchor, 0)
		iconBackground.Size = UDim2.fromOffset(size, size)
		iconBackground.BackgroundColor3 = HexToColor3(Settings.Color)
		iconBackground.BorderSizePixel = 0
		iconBackground.Parent = parent

		local iconBackgroundUICorner = Instance.new("UICorner")
		iconBackgroundUICorner.Name = "IconBackgroundUICorner"
		iconBackgroundUICorner.CornerRadius = UDim.new(0, math.floor(size * 0.28))
		iconBackgroundUICorner.Parent = iconBackground
		...
		icon.Size = UDim2.fromOffset(math.floor(size * 0.58), math.floor(size * 0.58))
		icon.BackgroundTransparency = 1
		icon.BorderSizePixel = 0
		icon.ZIndex = 2
		icon.Parent = iconBackground

		return iconBackground, size + 8
	else
		local icon = Instance.new("ImageLabel")
		icon.Name = "Icon"
		icon.Image = Settings.Icon
		icon.AnchorPoint = Vector2.new(0, vertAnchor)
		icon.Position = UDim2.new(0, leftOffset, vertAnchor, 0)
...
function MacLib:Window(Settings)
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			if Settings.Image or Settings.Icon then
				if Settings.Border then
					local tabIconBackground = Instance.new("Frame")
					tabIconBackground.Name = "TabIconBackground"
					tabIconBackground.BackgroundColor3 = HexToColor3(Settings.Color)
					tabIconBackground.BorderSizePixel = 0
					tabIconBackground.Size = UDim2.fromOffset(26, 26)
					tabIconBackground.LayoutOrder = 0
					tabIconBackground.Parent = tabSwitcher

					local tabIconBackgroundUICorner = Instance.new("UICorner")
					tabIconBackgroundUICorner.Name = "TabIconBackgroundUICorner"
					tabIconBackgroundUICorner.CornerRadius = UDim.new(0, 7)
					tabIconBackgroundUICorner.Parent = tabIconBackground

					tabImage = Instance.new("ImageLabel")
					tabImage.Name = "TabImage"
					tabImage.Image = Settings.Icon or Settings.Image
					tabImage.ImageTransparency = 0.1
					tabImage.AnchorPoint = Vector2.new(0.5, 0.5)
					tabImage.BackgroundTransparency = 1
					tabImage.BorderSizePixel = 0
					tabImage.Position = UDim2.fromScale(0.5, 0.5)
					tabImage.Size = UDim2.fromOffset(14, 14)
					tabImage.Parent = tabIconBackground
				else
					tabImage = Instance.new("ImageLabel")
					tabImage.Name = "TabImage"
					tabImage.Image = Settings.Icon or Settings.Image
			...
			local single = Instance.new("Frame")
			single.Name = "Single"
			single.AutomaticSize = Enum.AutomaticSize.Y
			single.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
			single.BackgroundTransparency = 1
			single.BorderColor3 = Color3.fromRGB(0, 0, 0)
			single.BorderSizePixel = 0
			single.LayoutOrder = -1
			single.Size = UDim2.new(1, 0, 0, 0)
			single.Visible = false

			local singleUIListLayout = Instance.new("UIListLayout")
			singleUIListLayout.Name = "SingleUIListLayout"
			singleUIListLayout.Padding = UDim.new(0, 15)
			singleUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
			singleUIListLayout.Parent = single

			single.Parent = elementsScrolling
			...
			function TabFunctions:Section(Settings)
				...
				section.Parent = Settings._parentOverride or (Settings.Side == "Single" and single or (Settings.Side == "Left" and left or right))
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					local _, buttonIconSpace = CreateElementIcon(button, Settings, 26, 0)
					if buttonIconSpace > 0 then
						local buttonInteractUIPadding = Instance.new("UIPadding")
						buttonInteractUIPadding.Name = "ButtonInteractUIPadding"
						buttonInteractUIPadding.PaddingLeft = UDim.new(0, buttonIconSpace)
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					local _, toggleIconSpace = CreateElementIcon(toggle, Settings, 26, 0)
					...
					toggleName.Position = UDim2.new(0, toggleIconSpace, 0.5, 0)
					toggleName.Size = UDim2.new(1, -72 - toggleIconSpace, 0, 0)
					...
					linkButton.Image = "rbxassetid://106205298246017"
					linkButton.ImageTransparency = 0.5
					linkButton.AutoButtonColor = false
					linkButton.AnchorPoint = Vector2.new(1, 0.5)
					linkButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					linkButton.BackgroundTransparency = 1
					linkButton.BorderColor3 = Color3.fromRGB(0, 0, 0)
					linkButton.BorderSizePixel = 0
					linkButton.Position = UDim2.new(1, -48, 0.5, 0)
					linkButton.Size = UDim2.fromOffset(16, 16)
					linkButton.Visible = false
					linkButton.ZIndex = 2
					linkButton.Parent = toggle
					...
					function ToggleFunctions:Link(config)
						...
						linkPanel.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
						linkPanel.BackgroundTransparency = 0.5
						linkPanel.BorderColor3 = Color3.fromRGB(0, 0, 0)
						linkPanel.BorderSizePixel = 0
						linkPanel.Size = UDim2.fromScale(1, 1)
						linkPanel.Visible = false

						local linkPanelUICorner = Instance.new("UICorner")
						linkPanelUICorner.Name = "LinkPanelUICorner"
						...
						local promptUIListLayout = Instance.new("UIListLayout")
						promptUIListLayout.Name = "PromptUIListLayout"
						...
						headerBar.BorderColor3 = Color3.fromRGB(0, 0, 0)
						headerBar.BorderSizePixel = 0
						headerBar.Size = UDim2.new(1, 0, 0, 44)

						local headerBarUIPadding = Instance.new("UIPadding")
						headerBarUIPadding.Name = "HeaderBarUIPadding"
						headerBarUIPadding.PaddingLeft = UDim.new(0, 16)
						headerBarUIPadding.PaddingRight = UDim.new(0, 16)
						headerBarUIPadding.Parent = headerBar

						local headerIcon = Instance.new("ImageLabel")
						headerIcon.Name = "HeaderIcon"
						headerIcon.Image = "rbxassetid://106205298246017"
						headerIcon.ImageTransparency = 0.3
						...
						headerTitle.Position = UDim2.new(0, 24, 0.5, 0)
						...
						headerDivider.BackgroundTransparency = 0.9
						headerDivider.BorderColor3 = Color3.fromRGB(0, 0, 0)
						headerDivider.BorderSizePixel = 0
						headerDivider.LayoutOrder = 1
						headerDivider.Size = UDim2.new(1, 0, 0, 1)
						headerDivider.Parent = prompt
						...
						contentScrolling.BottomImage = ""
						contentScrolling.CanvasSize = UDim2.new()
						contentScrolling.ScrollBarImageTransparency = 0.5
						contentScrolling.ScrollBarThickness = 1
						contentScrolling.TopImage = ""
						...
						local contentScrollingUIPadding = Instance.new("UIPadding")
						contentScrollingUIPadding.Name = "ContentScrollingUIPadding"
						contentScrollingUIPadding.PaddingBottom = UDim.new(0, 5)
						contentScrollingUIPadding.PaddingTop = UDim.new(0, 5)
						contentScrollingUIPadding.Parent = contentScrolling

						local contentScrollingUIListLayout = Instance.new("UIListLayout")
						contentScrollingUIListLayout.Name = "ContentScrollingUIListLayout"
						contentScrollingUIListLayout.Padding = UDim.new(0, 0)
						contentScrollingUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
						contentScrollingUIListLayout.Parent = contentScrolling

						prompt.Parent = linkPanel
						linkPanel.Parent = base

						-- Create the section inside the scrolling content via _parentOverride
						local linkSection = TabFunctions:Section({ Side = "Left", _parentOverride = contentScrolling })

						-- ── Open/close animation ──────────────────────────────────────
						local function makeLinkCanvas()
							local canvas = Instance.new("CanvasGroup")
							canvas.Name = "LinkPanelCanvas"
						...
						linkButton.MouseEnter:Connect(function()
							Tween(linkButton, TweenInfo.new(0.2, Enum.EasingStyle.Sine), { ImageTransparency = 0.2 }):Play()
						end)
						linkButton.MouseLeave:Connect(function()
							Tween(linkButton, TweenInfo.new(0.2, Enum.EasingStyle.Sine), { ImageTransparency = 0.5 }):Play()
						end)
						...
						-- Backdrop click-to-close (click outside the prompt card)
						linkPanel.InputBegan:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.MouseButton1 then
								local mousePos = UserInputService:GetMouseLocation()
								local pPos = prompt.AbsolutePosition
								local pSize = prompt.AbsoluteSize
								local onPrompt = mousePos.X >= pPos.X and mousePos.X <= pPos.X + pSize.X
									and mousePos.Y >= pPos.Y and mousePos.Y <= pPos.Y + pSize.Y
								if not onPrompt then
									transition(false)
								end
							end
						end)
						...
						-- ── Public API ────────────────────────────────────────────────
						function LinkFunctions:Open()
							transition(true)
						end

						function LinkFunctions:Close()
							transition(false)
						end

						function LinkFunctions:UpdateTitle(newTitle)
							headerTitle.Text = newTitle
						end

						-- Proxy all element-creation methods through the inner section
						setmetatable(LinkFunctions, { __index = linkSection })
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					local _, sliderIconSpace = CreateElementIcon(slider, Settings, 26, 0)
					...
					sliderName.Position = UDim2.new(0, sliderIconSpace, 0.5, 0)
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					local _, inputIconSpace = CreateElementIcon(input, Settings, 26, 0)
					...
					inputName.Position = UDim2.new(0, inputIconSpace, 0.5, 0)
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					local _, keybindIconSpace = CreateElementIcon(keybind, Settings, 26, 0)
					...
					keybindName.Position = UDim2.new(0, keybindIconSpace, 0.5, 0)
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					local _, dropdownIconSpace = CreateElementIcon(dropdown, Settings, 26, 15)
					...
					dropdownName.Position = UDim2.new(0, dropdownIconSpace, 0, 0)
					dropdownName.Size = UDim2.new(1, -20 - dropdownIconSpace, 0, 38)
					...
					-- ScrollingFrame that holds the actual options (max 150px, then scrolls)
					local optionsMaxHeight = 150
					...
					optionsScrollFrame.CanvasSize = UDim2.new()
					optionsScrollFrame.ScrollBarImageTransparency = 0.7
					optionsScrollFrame.ScrollBarThickness = 2
					optionsScrollFrame.BottomImage = ""
					optionsScrollFrame.TopImage = ""
					optionsScrollFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					optionsScrollFrame.BackgroundTransparency = 1
					optionsScrollFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
					optionsScrollFrame.BorderSizePixel = 0
					optionsScrollFrame.Size = UDim2.new(1, 0, 0, 0) -- set dynamically
					optionsScrollFrame.ClipsDescendants = true
					optionsScrollFrame.LayoutOrder = 1
					optionsScrollFrame.Parent = dropdownFrame

					local optionsScrollUIListLayout = Instance.new("UIListLayout")
					optionsScrollUIListLayout.Name = "OptionsScrollUIListLayout"
					optionsScrollUIListLayout.Padding = UDim.new(0, 5)
					optionsScrollUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
					optionsScrollUIListLayout.Parent = optionsScrollFrame
					...
					local function CalculateDropdownSize()
						...
						-- Measure visible options in the scroll frame
						local optionsTotalHeight = 0
						local visibleCount = 0
						for _, v in pairs(optionsScrollFrame:GetChildren()) do
							if not v:IsA("UIComponent") and not v:IsA("UIListLayout") and v.Visible then
								optionsTotalHeight += v.AbsoluteSize.Y
								visibleCount += 1
							end
						end
						local optionsSpacing = optionsScrollUIListLayout.Padding.Offset * math.max(0, visibleCount - 1)
						local rawOptionsHeight = optionsTotalHeight + optionsSpacing

						-- Search bar height (if visible)
						local searchHeight = 0
						if search.Visible then
							searchHeight = search.AbsoluteSize.Y + dropdownFrameUIListLayout.Padding.Offset
						end

						local clampedOptionsHeight = math.min(rawOptionsHeight, optionsMaxHeight)
						optionsScrollFrame.Size = UDim2.new(1, 0, 0, clampedOptionsHeight)

						return padding + searchHeight + clampedOptionsHeight
					end
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					local _, colorpickerIconSpace = CreateElementIcon(colorpicker, Settings, 26, 0)
					...
					colorpickerName.Position = UDim2.new(0, colorpickerIconSpace, 0.5, 0)
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					local _, headerIconSpace = CreateElementIcon(header, Settings, 26, 0)
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					local _, labelIconSpace = CreateElementIcon(label, Settings, 26, 0)
					...
					labelText.Position = UDim2.new(0, labelIconSpace, 0.5, 0)
					labelText.Size = UDim2.new(1, -labelIconSpace, 0, 0)
					...
					linkButton.BorderColor3 = Color3.fromRGB(0, 0, 0)
					linkButton.BorderSizePixel = 0
					linkButton.Position = UDim2.new(1, 0, 0.5, 0)
					linkButton.Size = UDim2.fromOffset(16, 16)
					linkButton.Visible = false
					linkButton.ZIndex = 2
					linkButton.Parent = label
					...
					function LabelFunctions:Link()
						...
						headerTitle.Text = LabelFunctions.Settings.Text
						headerTitle.RichText = true
						headerTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
						headerTitle.TextSize = 15
						headerTitle.TextTransparency = 0.3
				...
				function SectionFunctions:SubLabel(Settings, Flag)
					...
					local _, subLabelIconSpace = CreateElementIcon(subLabel, Settings, 18, 0)
					...
					subLabelText.AnchorPoint = Vector2.new(0, 0.5)
					...
					subLabelText.Position = UDim2.new(0, subLabelIconSpace, 0.5, 0)
					subLabelText.Size = UDim2.new(1, -subLabelIconSpace, 0, 0)
				...
				-- ======================================================================
				-- ===================== NEW ELEMENTS (custom addon) ===================
				-- ======================================================================

				-- Animated progress bar with percentage readout.
				function SectionFunctions:ProgressBar(Settings, Flag)
					...
					local ProgressBarFunctions = { Settings = Settings, Value = Settings.Default }
					...
					local title = Instance.new("TextLabel")
					title.Name = "Title"
					title.FontFace = Font.new(assets.interFont)
					title.Text = Settings.Name or "Progress"
					title.TextSize = 13
					title.TextXAlignment = Enum.TextXAlignment.Left
					title.BackgroundTransparency = 1
					...
					percentLabel.FontFace = Font.new(assets.interFont)
					percentLabel.TextSize = 13
					percentLabel.TextXAlignment = Enum.TextXAlignment.Right
					percentLabel.AnchorPoint = Vector2.new(1, 0)
					percentLabel.Position = UDim2.new(1, 0, 0, 0)
					percentLabel.BackgroundTransparency = 1
					percentLabel.Size = UDim2.new(0.3, 0, 1, 0)
					percentLabel.Parent = topRow
					RegisterThemeElement(percentLabel, "TextColor3", "TextMuted", "TextMutedTransparency")
					...
					track.LayoutOrder = 2
					track.Parent = holder
					RegisterThemeElement(track, "BackgroundColor3", "ElementBackground", "ElementBackgroundTransparency")
					...
					fill.Name = "Fill"
					fill.BackgroundColor3 = HexToColor3(Settings.Color)
					...
					fillGradient.Color = ColorSequence.new(HexToColor3(Settings.Color), Color3.new(1, 1, 1))
					...
					function ProgressBarFunctions:Set(value)
						value = math.clamp(value, Settings.Min, Settings.Max)
						ProgressBarFunctions.Value = value
						local alpha = (Settings.Max > Settings.Min) and (value - Settings.Min) / (Settings.Max - Settings.Min) or 0

						Tween(fill, TweenInfo.new(0.25, Enum.EasingStyle.Quad), { Size = UDim2.fromScale(alpha, 1) }):Play()
						percentLabel.Text = Settings.DisplayMethod == "Value"
							and tostring(math.floor(value))
							or (tostring(math.floor(alpha * 100)) .. "%")
					...
					function ProgressBarFunctions:SetVisibility(State)
						holder.Visible = State
					end
					function ProgressBarFunctions:Remove()
						holder:Destroy()
					end

					ProgressBarFunctions:Set(Settings.Default)

					if Flag then
						MacLib.Options[Flag] = ProgressBarFunctions
					end
					return ProgressBarFunctions
				end

				-- Colored callout/note box: Info, Success, Warning, Error.
				function SectionFunctions:Note(Settings, Flag)
					local NoteFunctions = { Settings = Settings }
					...
					local variantColor = variantColors[Settings.Type] or variantColors.Info

					local note = Instance.new("Frame")
					note.Name = "Note"
					note.AutomaticSize = Enum.AutomaticSize.Y
					note.BackgroundColor3 = variantColor
					note.BackgroundTransparency = 0.92
					...
					local sideBar = Instance.new("Frame")
					sideBar.Name = "SideBar"
					sideBar.BackgroundColor3 = variantColor
					sideBar.BorderSizePixel = 0
					...
					uIPadding.Parent = note

					local text = Instance.new("TextLabel")
					text.Name = "Text"
					text.FontFace = Font.new(assets.interFont)
					text.RichText = true
					...
					function NoteFunctions:UpdateText(Text)
						text.Text = Text
					end
					function NoteFunctions:SetVisibility(State)
						note.Visible = State
					end
					...
					if Flag then
						MacLib.Options[Flag] = NoteFunctions
					end
					return NoteFunctions
				end

				-- Segmented control: pick one option from a horizontal row of buttons.
				function SectionFunctions:ButtonGroup(Settings, Flag)
					Settings.Options = Settings.Options or {}
					Settings.Default = Settings.Default or Settings.Options[1]

					local ButtonGroupFunctions = { Settings = Settings, Value = Settings.Default }

					local holder = Instance.new("Frame")
					holder.Name = "ButtonGroup"
					holder.AutomaticSize = Enum.AutomaticSize.Y
					holder.BackgroundTransparency = 1
					holder.Size = UDim2.new(1, 0, 0, 0)
					holder.Parent = section
					...
					row.Name = "Row"
					row.Position = UDim2.new(0, 0, 0, Settings.Name and 22 or 0)
					row.AutomaticSize = Enum.AutomaticSize.Y
					row.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					row.BackgroundTransparency = 0.96
					row.Size = UDim2.new(1, 0, 0, 30)
					row.Parent = holder
					RegisterThemeElement(row, "BackgroundColor3", "ElementBackground", "ElementBackgroundTransparency")
					...
					rowPadding.PaddingBottom = UDim.new(0, 3)
					...
					rowListLayout.Padding = UDim.new(0, 3)
					rowListLayout.Parent = row

					local optionButtons = {}

					local function Refresh()
						for _, entry in ipairs(optionButtons) do
							local isSelected = entry.Value == ButtonGroupFunctions.Value
							Tween(entry.Button, TweenInfo.new(0.18, Enum.EasingStyle.Sine), {
								BackgroundTransparency = isSelected and 0.85 or 1
							}):Play()
							Tween(entry.Label, TweenInfo.new(0.18, Enum.EasingStyle.Sine), {
								TextTransparency = isSelected and 0 or 0.5
							}):Play()
						end
					end

					for i, optionValue in ipairs(Settings.Options) do
						local optButton = Instance.new("TextButton")
						optButton.Name = "Option_" .. tostring(i)
						optButton.Text = ""
						optButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
						optButton.BackgroundTransparency = 1
						optButton.BorderSizePixel = 0
						optButton.Size = UDim2.new(1 / #Settings.Options, 0, 1, 0)
						optButton.LayoutOrder = i
						optButton.Parent = row
						RegisterThemeElement(optButton, "BackgroundColor3", "ElementBackground")
						...
						optLabel.FontFace = Font.new(assets.interFont)
						optLabel.Text = tostring(optionValue)
						optLabel.TextSize = 12
						optLabel.TextTransparency = 0.5
						optLabel.TextTruncate = Enum.TextTruncate.AtEnd
						optLabel.BackgroundTransparency = 1
						optLabel.Size = UDim2.fromScale(1, 1)
						optLabel.Parent = optButton
						RegisterThemeElement(optLabel, "TextColor3", "TextPrimary")

						table.insert(optionButtons, { Button = optButton, Label = optLabel, Value = optionValue })

						optButton.MouseButton1Click:Connect(function()
							ButtonGroupFunctions.Value = optionValue
							Refresh()
							if Settings.Callback then
								Settings.Callback(optionValue)
							end
						end)
					end
					...
					function ButtonGroupFunctions:Set(value)
						ButtonGroupFunctions.Value = value
						Refresh()
					end
					function ButtonGroupFunctions:SetVisibility(State)
						holder.Visible = State
					end
					function ButtonGroupFunctions:Remove()
						holder:Destroy()
					end

					if Flag then
						MacLib.Options[Flag] = ButtonGroupFunctions
					end
					return ButtonGroupFunctions
				end

				-- ======================================================================
				-- =================== END NEW ELEMENTS (custom addon) =================
				-- ======================================================================
	...
	-- ==========================================================================
	-- ========================= NEW UTILITIES (custom addon) ==================
	-- ==========================================================================

	-- One-line shorthand for quick, low-ceremony notifications.
	-- Type can be "Info" | "Success" | "Warning" | "Error" (defaults to Info),
	-- controlling the accent color used on the notification.
	function WindowFunctions:Toast(Text, Type, Lifetime)
		local toastColors = {
			Info    = Color3.fromRGB(90, 170, 255),
			Success = Color3.fromRGB(90, 220, 140),
			Warning = Color3.fromRGB(255, 190, 80),
		...
		WindowFunctions:Notify({
			...
			Color       = toastColors[Type] or toastColors.Info,
		})
	end

	-- Small celebratory confetti burst overlaid on the whole screen.
	-- Purely cosmetic, self-cleaning, no persistent state.
	function WindowFunctions:Celebrate(Amount)
		Amount = Amount or 40
		local confettiColors = {
			Color3.fromRGB(255, 99, 132), Color3.fromRGB(255, 205, 86),
			Color3.fromRGB(75, 192, 192), Color3.fromRGB(153, 102, 255),
			Color3.fromRGB(255, 159, 64),
		}

		local burst = Instance.new("Frame")
		burst.Name = "ConfettiBurst"
		...
		for i = 1, Amount do
			...
			piece.Size = UDim2.fromOffset(math.random(4, 8), math.random(8, 14))
			piece.Position = UDim2.new(math.random(0, 100) / 100, 0, 0, -20)
			piece.Rotation = math.random(0, 360)
			piece.ZIndex = 10
			piece.Parent = burst

			local dropTime = math.random(15, 25) / 10
			Tween(piece, TweenInfo.new(dropTime, Enum.EasingStyle.Quad), {
				Position = UDim2.new(piece.Position.X.Scale, 0, 1, 20),
				Rotation = piece.Rotation + math.random(180, 720),
			}):Play()
			Tween(piece, TweenInfo.new(dropTime * 0.3, Enum.EasingStyle.Linear), {
	...
	-- ==========================================================================
	-- ======================= END NEW UTILITIES (custom addon) ================
	-- ==========================================================================
...
function MacLib:Demo()
	...
	local Tabs = {
		Home     = TabGroup:Tab({ Name = "Home",     Description = "Overview and main features", Image = "rbxassetid://18821914323", Border = true }),
	...
	HomeSection:Header({ Name = "Welcome", Icon = "rbxassetid://78791133479661", Border = true })
	...
	local sonion = VisRightSection:Label({ Text = "Primary color applies to main UI elements." })
	sonion:Link()
	VisRightSection:SubLabel({ Text = "Accent color is used for highlights and borders." })

	-- ==========================================================================
	-- ==================== NEW FEATURES DEMO (custom addon) ===================
	-- ==========================================================================

	VisRightSection:Divider()
	VisRightSection:Header({ Name = "New Elements" })

	VisRightSection:Note({
		Type = "Info",
		Text = "This is an <b>info</b> note. Use it for tips or context.",
	})
	VisRightSection:Note({
		Type = "Warning",
		Text = "This is a <b>warning</b> note.",
	})

	local DownloadProgress = VisRightSection:ProgressBar({
	...
	VisRightSection:Button({
		...
		Callback = function()
			task.spawn(function()
				...
				Window:Toast("Download complete!", "Success")
				Window:Celebrate()
			end)
		end,
	})

	VisRightSection:ButtonGroup({
		Name     = "Quality Preset",
		Options  = { "Low", "Medium", "High", "Ultra" },
		Default  = "Medium",
		Callback = function(value)
			Window:Toast("Quality set to " .. value, "Info", 3)
		end,
	}, "QualityPreset")

	-- ==========================================================================
	-- ================== END NEW FEATURES DEMO (custom addon) =================
	-- ==========================================================================
